...
const notableBirthEvents = notablePeople
  ...
  .map((person) => {
    ...
    return {
      ...
      summary_bg: `Биографичният запис отбелязва раждането на ${person.name_bg} в Пловдив през ${person.birth_year} г. Ролите в базата са: ${roleList(person.roles, "bg")}.`,
      summary_en: `${person.name_en} was born in Plovdiv in ${person.birth_year}. Knowledge-base roles: ${roleList(person.roles, "en")}.`,
